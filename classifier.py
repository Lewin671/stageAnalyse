...
class Classifier(BaseModelClassifier):
    ...
    def __init__(
        ...
        super(Classifier, self).__init__(*_, **__)
    ...
    def predict_with_object(self, frame: np.ndarray) -> str:
        ...
        predict_result = self._model.predict(frame)
        print("predict result: ", predict_result)
        return str(np.argmax(predict_result, axis=1)[0])
